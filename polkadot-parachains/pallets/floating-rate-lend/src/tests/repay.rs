...
#[test]
fn floating_lend_repay_zero_balance() {
    ExtBuilder::default()
        .build()
        .execute_with(|| {
            assert_noop!(FloatingRateLend::repay(Origin::signed(ROOT), 0, 0), Error::<Runtime>::BalanceTooLow);
        });
}

#[test]
fn floating_lend_repay_not_enabled() {
    ExtBuilder::default()
        .build()
        .execute_with(|| {
            ...
            assert_noop!(
                FloatingRateLend::repay(Origin::signed(ROOT), 0, 100),
...
#[test]
fn floating_lend_repay_no_borrow() {
    ExtBuilder::default()
        .build()
        .execute_with(|| {
            ...
            assert_noop!(
                FloatingRateLend::repay(Origin::signed(ROOT), 0, 2 * BALANCE_ONE),
...
#[test]
fn floating_lend_repay_success() {
    ExtBuilder::default()
        .build()
        .execute_with(|| {
            ...
            assert_ok!(FloatingRateLend::repay(Origin::signed(ROOT), 2, 200 * BALANCE_ONE));

            // check supplies
            let user = PoolUserDebts::<Runtime>::get(pool_2.id(), ROOT.clone()).unwrap();
            assert_eq!(user.amount(), FixedU128::from(600));

            let pool = PoolStorage::<Runtime>::get(pool_2.id()).unwrap();
            assert_eq!(pool.debt(), FixedU128::from(1800));

            // this is just to test we can withdraw if within liquidation threshold
            assert_ok!(FloatingRateLend::repay(Origin::signed(ROOT), 2, 800 * BALANCE_ONE));
            let user = PoolUserDebts::<Runtime>::get(pool_2.id(), ROOT.clone());
            assert_eq!(user.is_none(), true);

            let pool = PoolStorage::<Runtime>::get(pool_2.id()).unwrap();
            assert_eq!(pool.debt(), FixedU128::from(1200));
        });
}
