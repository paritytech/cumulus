...
use hex_literal::hex;
use relay_chain::v3::HrmpChannelId;
use sp_core::H256;
use sp_runtime::{
	testing::Header,
...
fn events() {
	BlockTests::new()
		.with_relay_sproof_builder(|_, block_number, builder| {
			if block_number > 123 {
				builder.upgrade_go_ahead = Some(relay_chain::v3::UpgradeGoAhead::GoAhead);
			}
		})
		.add_with_post_test(
			123,
...
fn aborted_upgrade() {
	BlockTests::new()
		.with_relay_sproof_builder(|_, block_number, builder| {
			if block_number > 123 {
				builder.upgrade_go_ahead = Some(relay_chain::v3::UpgradeGoAhead::Abort);
			}
		})
		.add(123, || {
			assert_ok!(System::set_code(RawOrigin::Root.into(), Default::default()));
