...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_validation_data(
			...
			match upgrade_go_ahead_signal {
				Some(relay_chain::v3::UpgradeGoAhead::GoAhead) => {
					assert!(
				...
				Some(relay_chain::v3::UpgradeGoAhead::Abort) => {
	...
	pub(super) type UpgradeRestrictionSignal<T: Config> =
		StorageValue<_, Option<relay_chain::v3::UpgradeRestriction>, ValueQuery>;
	...
	pub(super) type HrmpWatermark<T: Config> =
		StorageValue<_, relay_chain::v3::BlockNumber, ValueQuery>;
...
impl<T: Config> Pallet<T> {
	...
	fn process_inbound_horizontal_messages(
		...
		relay_parent_number: relay_chain::v3::BlockNumber,
	) -> Weight {
