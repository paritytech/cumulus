...
pub use westmint_runtime::{
	...
	RuntimeEvent, SessionKeys, System, TrustBackedAssets,
};
...
#[test]
fn test_unpaid_bridged_transact_with_remark_works() {
	ExtBuilder::<Runtime>::default()
		.with_collators(vec![AccountId::from(ALICE)])
		.with_session_keys(vec![(
			AccountId::from(ALICE),
		...
		.execute_with(|| {
			...
			// check Event::Remarked
			let events = System::events();
			log::trace!(target: "xcm::execute", "events: {:?}", events);
