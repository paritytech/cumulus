...
fn swap_locally_on_chain_using_foreign_assets() {
	...
	Statemine::fund_accounts(vec![
		(StatemineSender::get(), 5_000_000), // An account to swap dot for something else.
		(sov_penpal_on_statemine.clone(), 10_000_000_000_000_000),
	]);
	...
	let call_foreign_assets_create =
		<Statemine as Para>::RuntimeCall::ForeignAssets(pallet_assets::Call::<
	...
	Statemine::execute_with(|| {
		...
		assert_ok!(<Statemine as StateminePallet>::ForeignAssets::mint(
			...
			foreign_asset1_at_statemine,
			sov_penpal_on_statemine.clone().into(),
			42_000_000_000_000,
		));
		...
		// 5. Add liquidity:
		assert_ok!(<Statemine as StateminePallet>::AssetConversion::add_liquidity(
			<Statemine as Parachain>::RuntimeOrigin::signed(sov_penpal_on_statemine.clone()),
			asset_native,
			foreign_asset1_at_statemine,
			1_000_000_000, // 33_333_333 min ksm
			2_000_000_000, // 1_000_000_000 min
		...
		// 6. Swap!
		let path =
			BoundedVec::<_, _>::truncate_from(vec![asset_native, foreign_asset1_at_statemine]);
		//TODO: this should be done by some other account!
		assert_ok!(<Statemine as StateminePallet>::AssetConversion::swap_exact_tokens_for_tokens(
			// <Statemine as Parachain>::RuntimeOrigin::signed(sov_penpal_on_statemine.clone()),
			<Statemine as Parachain>::RuntimeOrigin::signed(StatemineSender::get()),
