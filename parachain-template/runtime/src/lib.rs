...
/// How many parachain blocks are processed by the relay chain per slot. Limits the
/// number of blocks authored per slot. Our slots are 2 relay-chain slots, so we expect
/// a velocity of 2 blocks per slot.
